...
# modify these paths if you are using different paths to urdf files
...
JOINT_NAMES = ['arm_elbow_joint', 'arm_shoulder_lift_joint', 'arm_shoulder_pan_joint', 'arm_wrist_1_joint',
               'arm_wrist_2_joint', 'arm_wrist_3_joint']  # , 'gripper_finger_joint', a fake one, not currently useful
# end effector name
GRIPPER = 'gripper_robotiq_arg2f_base_link'
# since adjacent links must be connected, we need disable them for self collision check 
...
JOINT_ACTION_SERVER = '/scaled_pos_joint_traj_controller/follow_joint_trajectory'
...
class Tahoma:
    ...
    def get_actual_state(self, msg: JointState):
        # callback function for listening to robot's actual state
        # because we don't want the last joint state 'gripper_finger_joint', the fake one,
        # we only select the first 6 states.
    ...
    def move_to_pose_goal(self,
        ...
        """
        Moves the end-effector to an end pose, using pybullet planning.

        Args:
            end_pose: geometry_msgs/PoseStamped. The goal pose for the gripper
            max_plan_time: float. The maximum allowed plan time
            execution_timeout: float. The maximum allowed time to wait for the real robot to execute path
            tolerance: float. The tolerance of joint angles in rad
            orientation_constraint: moveit_msgs/OrientationConstraint. An orientation constraint for the entire path.

        Returns:
            true if real robot successfully moves to end pose; false otherwise
        """
        # synchronize to real robot
        self.synchronize()
        ...
        path = plan_motion_from_to(self._pb_robot, self._joint_indices, self._actual_joint_state, end_joint_state,
                                   ...
                                   algorithm='rrt')
        if path is None:
            cprint('FAILUER: pybullet cannot find any path', 'red')
            return False

        # meanwhile, move arm in pybullet sim
        # comment this out if you don't want to see the effect in pybullet
        ...
        if result is not None:
            ...
            return all_close(self._actual_joint_state, end_joint_state, tolerance)
...
def all_close(state1: list, state2: list, threshold):
    """
    return whether two state vectors' euclidean distance is within threshold
    """
...
def mark_target(position: list):
    """
    place a red box at position, for debugging purpose
    """
...
def random_restarts(solve_fn, start, goal, distance_fn, sample_fn, extend_fn, collision_fn,
    ...
    for attempt in pp.irange(restarts + 1):
        ...
        # if find a low-cost path, exit the loop
